...
async function loadToolkitApp() {
  ...
  const uiModule = new vm.SyntheticModule(['ToolkitUI'], function () {
    this.setExport('ToolkitUI', { updatePreview: (type, content) => previews.push({ type, content }) });
  }, { context });
  const utilsModule = new vm.SourceTextModule(await readFile(new URL('../web/tk_workflow_utils.js', import.meta.url), 'utf8'), { context });
  const appModule = new vm.SourceTextModule(await readFile(new URL('../web/tk_app.js', import.meta.url), 'utf8'), { context });
  ...
  return { app: appModule.namespace.ToolkitApp, savedHistory, previews };
}
